...
func TestInit(t *testing.T) {
	...
	if l := len(paths); l != 0 {
		t.Fatalf("Fresh store should be empty after init (len=%d)", l)
	}
}
...
func TestCopyNewPath(t *testing.T) {
	...
	if images, err := store.List("foo"); err != nil {
		...
		t.Fatalf("Wrong number of images at source path (should be %d, not %d)", 1, l)
	}
	// Check number of images at destination path
	if images, err := store.List("bar"); err != nil {
		t.Fatal(err)
	} else if l := len(images); l != 1 {
		t.Fatalf("Wrong number of images at destination path (should be %d, not %d)", 1, l)
	}
...
func TestCopySameName(t *testing.T) {
	...
	_, err = src.Copy("foo")
	if err == nil {
		t.Fatalf("Copying an image to the same patch twice should fail.")
	}
}
...
func TestMountPoint(t *testing.T) {
	...
	if mountpoint.Root != "/tmp/a" {
		t.Fatalf("Wrong mountpoint root (should be %s, not %s)", "/tmp/a", mountpoint.Root)
	}
	if mountpoint.Rw != "/tmp/b" {
		t.Fatalf("Wrong mountpoint root (should be %s, not %s)", "/tmp/b", mountpoint.Rw)
	}
}
...
func TestMountpointDuplicateRoot(t *testing.T) {
	...
	if _, err = image.Mountpoint("/tmp/a", "/tmp/foobar"); err == nil {
		t.Fatalf("Duplicate mountpoint root should fail")
	}
}
...
func TestMount(t *testing.T) {
	...
	if !mountpoint.Mounted() {
		t.Fatalf("Mountpoint not mounted")
	}
	// There should be one mountpoint registered
	if mps, err := image.Mountpoints(); err != nil {
		t.Fatal(err)
	} else if len(mps) != 1 {
		t.Fatalf("Wrong number of mountpoints registered (should be %d, not %d)", 1, len(mps))
	}
	...
	if mps, err := image.Mountpoints(); err != nil {
		...
		t.Fatalf("Wrong number of mountpoints registered (should be %d, not %d)", 0, len(mps))
	}
